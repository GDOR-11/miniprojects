(() => {
  ...
  // src/gabriels_point/point.ts
  var Point = class _Point {
    constructor(x, y) {
      this.x = typeof x == "number" ? new decimal_default(x) : x;
      this.y = typeof y == "number" ? new decimal_default(y) : y;
    }
    set(p) {
      this.x = p.x;
      this.y = p.y;
    ...
    plus(p) {
      return new _Point(this.x.plus(p.x), this.y.plus(p.y));
    }
    minus(p) {
      return new _Point(this.x.minus(p.x), this.y.minus(p.y));
    }
    times(x) {
      if (typeof x === "number") x = new decimal_default(x);
      return new _Point(this.x.times(x), this.y.times(x));
    }
    dot(p) {
  ...
  // src/gabriels_point/index.ts
  decimal_default.config({ precision: 32 });
  var canvas = document.getElementById("canvas");
  var ctx = canvas.getContext("2d");
  var A = new Point(100, window.innerHeight / 2);
  var B = new Point(window.innerWidth / 2, window.innerHeight / 2).minus(new Point(0, window.innerWidth - 200).times(new decimal_default(3).sqrt().div(2)));
  var C = new Point(window.innerWidth - 100, window.innerHeight / 2);
  function project(P2, A2, B2) {
    let p = P2.minus(A2);
    let b = B2.minus(A2);
    let mult = p.dot(b).div(b.lenSq());
    return A2.plus(b.times(mult));
  }
  function orthic_triangle(A2, B2, C2) {
    return [
      project(A2, B2, C2),
      project(B2, A2, C2),
      project(C2, A2, B2)
  ...
  function draw_points(points, color = "white", radius = 5) {
    ctx.fillStyle = color;
  ...
  function draw_polygon(points, fill_color = "white", border_color = "white", border_width = 5) {
  ...
  window.onpointerdown = (e) => {
    ...
    let is_selected = (p) => p.minus(pointer).len().lt(20);
    selected_point = null;
